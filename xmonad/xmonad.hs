...
myKeyBindings =
  [
    ...
    , ((myModMask .|. mod1Mask, xK_space), spawn "dmenu_run")
    , ((myModMask, xK_u), focusUrgent)
    , ((controlMask .|. shiftMask, xK_m), spawn "amixer -q -D pulse sset Master toggle")
    , ((controlMask .|. shiftMask, xK_Down), spawn "amixer -q set Master 10%-")
    , ((controlMask .|. shiftMask, xK_Up), spawn "amixer -q set Master 10%+")
  ]
...
myKeys = myKeyBindings ++
  ...
  [
    ((controlMask .|. shiftMask, xK_Right), spawn "playerctl next"),
    ((controlMask .|. shiftMask, xK_Left), spawn "playerctl previous"),
    ((controlMask .|. shiftMask, 0x20), spawn "playerctl play-pause")
  ]
  --[
